    ...
    return <div className="inline-flex border py-1 rounded-lg bg-gray-200 ">
        {tabs.map((tab, index) =>
            <div key={index}>
                ...
                <label className="radio-label p-2 rounded capitalize transition-all duration-500 ease-in-out" htmlFor={tab + name} style={{ ...({ "--color": secondaryColor } as React.CSSProperties) }}>{tab}</label >
            </div>
        )}
    </div>;
});
